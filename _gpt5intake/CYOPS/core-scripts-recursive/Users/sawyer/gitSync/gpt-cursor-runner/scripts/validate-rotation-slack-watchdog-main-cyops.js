...
const ROOT_LOG = '/Users/sawyer/gitSync/.cursor-cache/ROOT/.logs/validation-tests.log';
...
function testSlackFallback() {
  try {
    ...
    monitor.sendCorrelatedAlert('test_alert', 'validation', 'forcing slack fail', 'warning');
    // Expect LOCAL_ALERTS file to be written
    const fallback = path.join(ROOT_LOGS_DIR, 'LOCAL_ALERTS');
    setTimeout(() => {
      if (fs.existsSync(fallback)) wlog('PASS: Slack fallback wrote LOCAL_ALERTS');
...
function paritySweep() {
  try {
    ...
    const blocks = [
      JSON.stringify({ role: 'command_patch', target: 'CYOPS', blockId: 'cyops-parity-1' }),
      JSON.stringify({ role: 'command_patch', target: 'MAIN', blockId: 'main-parity-1' }),
    ...
    const cyopsOk = expectCYOPS.every(f => fs.existsSync(path.join(CYOPS_PATCH_DIR, f)));
    const mainOk = expectMAIN.every(f => fs.existsSync(path.join(MAIN_PATCH_DIR, f)));
    if (cyopsOk && mainOk) wlog('PASS: MAIN/CYOPS parity sweep wrote files');
    else wlog('FAIL: MAIN/CYOPS parity sweep missing files');
    // cleanup
    expectCYOPS.forEach(f => { try { fs.unlinkSync(path.join(CYOPS_PATCH_DIR, f)); } catch {} });
    expectMAIN.forEach(f => { try { fs.unlinkSync(path.join(MAIN_PATCH_DIR, f)); } catch {} });
  } catch (e) {
...
function watchdogKillRecoverNote() {
  ...
  wlog('INFO: Watchdog validation requires PM2-managed kill/restart; manual follow-up recommended');
}
