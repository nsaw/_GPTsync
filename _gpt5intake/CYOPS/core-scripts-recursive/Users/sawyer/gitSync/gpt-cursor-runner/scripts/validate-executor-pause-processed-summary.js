...
(function run(){
  wlog('=== VALIDATION: Executor pause keeps patch unprocessed; ALLOW_PROCEED processes ===');
  ...
  try { execSync(`python3 ${py} --check-interval 1`, { timeout: 3000 }); } catch {}
  ...
  try { execSync(`python3 ${py} --check-interval 1`, { timeout: 20000 }); } catch {}
  ...
  if (!exists(summary)) {
    try {
      const files = fs.readdirSync(SUMMARIES_DIR).filter(f => f.startsWith('summary-') && f.endsWith('.md'));
