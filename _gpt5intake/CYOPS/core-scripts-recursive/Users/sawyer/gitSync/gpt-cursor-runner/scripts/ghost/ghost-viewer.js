...
app.get('/health', (_req, res) => {
  res.json({ status: 'healthy', service: 'ghost-viewer', port: PORT, timestamp: new Date().toISOString() });
});
...
app.get('/viewer', (req, res) => {
  ...
  try {
    const files = fs.existsSync(summariesPath) ? fs.readdirSync(summariesPath) : [];
    const items = files
      .filter((f) => f.endsWith('.md'))
      .map((f) => `<li><a href="/viewer/file?zone=${zone}&file=${encodeURIComponent(f)}">${f}</a></li>`) 
      .join('');
    res.send(`<html><body><h2>${zone} Summaries</h2><ul>${items}</ul></body></html>`);
  } catch (_e) {
