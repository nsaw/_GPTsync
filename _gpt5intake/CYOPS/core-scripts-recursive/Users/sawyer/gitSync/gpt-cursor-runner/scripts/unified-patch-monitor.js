...
const UNIFIED_MONITOR_STATUS_FILE = path.join(ROOT_LOGS_DIR, 'unified-monitor-status.json');
const ALERT_CORRELATION_FILE = path.join(ROOT_LOGS_DIR, 'alert-correlation.json');
...
function loadAlertCorrelationState() {
  try {
    ...
    console.error('[MONITOR] Failed to load alert correlation state:', error.message);
  }
}
...
function saveAlertCorrelationState() {
  try {
    ...
    fs.writeFileSync(ALERT_CORRELATION_FILE, JSON.stringify(stateToSave, null, 2));
  } catch (error) {
    console.error('[MONITOR] Failed to save alert correlation state:', error.message);
  }
}
...
function createLocalAlertFallback(alertData) {
  try {
    ...
    console.error('[MONITOR] Failed to create local alert fallback:', error.message);
  }
}
...
function updateDashboardAlert(alertData) {
  try {
    ...
    console.error('[MONITOR] Failed to update dashboard alerts:', error.message);
  }
}
...
function monitorStuckPatches() {
  try {
    ...
    const stuckFiles = fs.readdirSync(stuckDir)
...
function createStuckPatchesDashboard(stuckAlert) {
  try {
    const dashboardFile = path.join(ROOT_LOGS_DIR, 'stuck-patches-dashboard.json');
    const dashboardData = {
    ...
    console.error('[MONITOR] Failed to create stuck patches dashboard:', error.message);
  }
}
...
function writeUnifiedLog(logFile, message) {
  try {
        ...
        if (rotatedFiles.length >= 10) {
          ...
          filesToDelete.forEach(file => {
            try {
              ...
              console.error(`[MONITOR] Failed to purge old log ${file}:`, e.message);
            }
          });
        }
    ...
    console.error(`[MONITOR] Failed to write to log ${logFile}:`, error.message);
...
function handleLogWriteFailure(logFile, error, originalMessage) {
  ...
  try {
    ...
    console.error('[MONITOR] CRITICAL: Even fallback logging failed:', fallbackError.message);
  }
  ...
  try {
    ...
    console.error('[MONITOR] CRITICAL: Failed to create alert file:', alertError.message);
  }
  
  console.error('[MONITOR] 🚨 CRITICAL: Log write failure - system may be degraded');
}
...
function startMonitoring() {
  ...
  fs.writeFileSync(UNIFIED_MONITOR_STATUS_FILE, JSON.stringify(statusData, null, 2));
  
  // Start monitoring loop
  setInterval(() => {
    try {
      // Heartbeat checks
      checkHeartbeat(path.join(ROOT_LOGS_DIR, 'ghost-bridge-status.json'), 60000, 'ghost-bridge');
      checkHeartbeat(path.join(ROOT_LOGS_DIR, 'patch-executor-status.json'), 60000, 'patch-executor');
      ...
      fs.writeFileSync(UNIFIED_MONITOR_STATUS_FILE, JSON.stringify(updatedStatus, null, 2));
      
    } catch (error) {
      console.error('[MONITOR] Monitoring loop error:', error.message);
      sendCorrelatedAlert('monitoring_error', 'unified-monitor', error.message, 'error');
    }
  }, 30000); // 30 seconds
...
function checkHeartbeat(statusFile, staleMs, component) {
  try {
    if (!fs.existsSync(statusFile)) {
      sendCorrelatedAlert('heartbeat_missing', component, `${component} status file missing`, 'warning');
      return;
    }
    const data = JSON.parse(fs.readFileSync(statusFile, 'utf8'));
    const ts = data.timestamp ? Date.parse(data.timestamp) : NaN;
    if (Number.isNaN(ts)) {
      sendCorrelatedAlert('heartbeat_invalid', component, `${component} timestamp invalid`, 'warning');
      return;
    }
    const age = Date.now() - ts;
    if (age > staleMs) {
      sendCorrelatedAlert('heartbeat_stale', component, `${component} heartbeat stale ${Math.floor(age/1000)}s`, 'error');
    }
  } catch (e) {
    sendCorrelatedAlert('heartbeat_check_failed', component, e.message, 'error');
  }
}
