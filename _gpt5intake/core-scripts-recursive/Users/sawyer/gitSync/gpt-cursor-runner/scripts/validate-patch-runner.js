...
const LOG_FILE = '/Users/sawyer/gitSync/.cursor-cache/ROOT/.logs/validation-tests.log';
...
async function main() {
  ...
  try {
    ...
    if (!res.processed) throw new Error(`processPatchBlock returned false: ${res.reason}`);
    ...
    failures++; writeLog(`FAIL: JSON patch extraction - ${e.message}`);
  }
  ...
  try {
    ...
    failures++; writeLog(`FAIL: YAML patch extraction - ${e.message}`);
  }

  // Test 3: Heartbeat writes
  try {
    const statusFile = '/Users/sawyer/gitSync/.cursor-cache/ROOT/.logs/ghost-bridge-status.json';
    bridge.emitHeartbeat();
    bridge.emitHeartbeat();
    if (!fs.existsSync(statusFile)) throw new Error('ghost-bridge-status.json not found');
    writeLog('PASS: Heartbeat file write');
  } catch (e) {
    failures++; writeLog(`FAIL: Heartbeat write - ${e.message}`);
  }

  // Test 4: Stale patch detection
  try {
    if (!fs.existsSync(CYOPS_PATCH_DIR)) fs.mkdirSync(CYOPS_PATCH_DIR, { recursive: true });
    ...
    failures++; writeLog(`FAIL: Stale patch detection - ${e.message}`);
  }

  if (failures === 0) writeLog('=== VALIDATION COMPLETE: ALL PASS ===');
  else writeLog(`=== VALIDATION COMPLETE: ${failures} FAILURES ===`);
}
