...
function simulateLogFailurePauseResumeBridge() {
  try {
    ...
    const res = bridge.processPatchBlock(JSON.stringify({ role: 'command_patch', target: 'CYOPS', blockId: 'pause-test' }), 'pause');
    if (res.processed === false && (res.reason === 'paused_due_to_log_failure' || res.reason === 'paused')) {
    ...
    const res2 = bridge.processPatchBlock(JSON.stringify({ role: 'command_patch', target: 'CYOPS', blockId: 'resume-test' }), 'resume');
    if (res2.processed) wlog('PASS: Bridge resumed with ALLOW_PROCEED');
    else wlog('FAIL: Bridge did not resume with ALLOW_PROCEED');
    // cleanup flags
    try { fs.unlinkSync(inject); } catch {}
    try { fs.unlinkSync(allow); } catch {}
...
function pm2KillRestartValidation() {
  try {
    ...
    wlog('INFO: PM2 kill/restart validation scaffold: use the following commands non-blocking');
    wlog('CMD: { timeout 15s pm2 restart ghost-bridge & } >/dev/null 2>&1 & disown');
    wlog('CMD: { timeout 15s pm2 restart patch-executor & } >/dev/null 2>&1 & disown');
    wlog('CMD: { timeout 15s pm2 logs --lines 30 ghost-bridge & } >/dev/null 2>&1 & disown');
  } catch (e) {
